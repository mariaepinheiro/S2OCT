...
function S2OCT(Xl,Xu,ma,τ,p,C,M,maxtime,bd) ###Xl: first ma belongs to A.
    ρ = 2^p -1
    p1 = 2^p
    p2 = Int64(p1/2)
    ml,n = size(Xl)
    ...
    if fix == 1
        @constraint(model, [i=1:ml,d=1:d0], dot(w1[:,d],Xl[i,:]) - γ1[d] + 1≤ yℓ[i,d])
        @constraint(model, [i=1:ml,d=1:d0], -dot(w1[:,d],Xl[i,:]) + γ1[d]+1 ≤ yg[i,d] )
        @constraint(model, [ix=1:mu,d=1:d0], dot(w1[:,d],Xu[ix,:]) -γ1[d] ≤ -1+zg[ix,d]*M)
        @constraint(model, [ix=1:mu,d=1:d0], dot(w1[:,d],Xu[ix,:]) -γ1[d] ≥ 1 -(1-zg[ix,d])*M)
    end
    #if p ≥ 3
